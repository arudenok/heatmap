...
- **USER** — просматривает реестр, может отправить новый инструмент («Добавить инструмент»).
  ...
  вкладках, пока администратор её не одобрит. Пользователь видит все свои инструменты
  (в любом статусе) в блоке «Мои инструменты» и может их редактировать или отозвать.
  Правка уже опубликованного или отклонённого инструмента автором (не администратором)
  отправляет его на повторную модерацию; причина отклонения (если она была) при этом
  сбрасывается. При отклонении администратор указывает причину — автор видит её в
  уведомлении и в блоке «Мои инструменты».
- **ADMIN** — всё то же самое, плюс раздел **«Администрирование»**:
  - модерация заявок (одобрить / отклонить с указанием причины),
  - редактирование инструмента в любом статусе без повторной модерации,
...
| POST   | `/api/auth/login`                 | публичный      | Вход, логин Сигма + пароль                 |
...
| PATCH  | `/api/tools/{id}`                 | владелец/админ | Изменить инструмент (правка автором PUBLISHED/REJECTED → снова PENDING) |
...
| POST   | `/api/admin/tools/{id}/reject`    | ADMIN          | Отклонить заявку (тело: `{ "reason": "..." }`, обязательно) |
